...
export function UserTable({ users, setUsers }) {
  ...
  <div className="overflow-y-auto hide-scroll w-full max-h-[600px] ">
    <table className="w-full text-center border-collapse" border={1} cellSpacing={0}>
      ...
      <tbody>
        {users.map((e, ind) => (
          <tr key={e._id} className="border-b border-gray-600">
            ...
            <td className="p-3">{<EditUser user={e} setUsers={setUsers} />}</td>
            <td className="p-3">{<DeleteUserButton setUsers={setUsers} user={e} />}</td>
          </tr>
        ))}
      </tbody>
    </table>
